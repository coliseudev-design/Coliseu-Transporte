...
def print_summary(results: List[dict]):
    ...
    # Find if any required check failed
    all_checks = CORE_CHECKS + PERFORMANCE_CHECKS
    required_names = {c[0] for c in all_checks if c[2]}
    critical_failed = sum(1 for r in results if not r["passed"] and not r.get("skipped") and r["name"] in required_names)
    
    if critical_failed > 0:
        print_error(f"{critical_failed} critical check(s) FAILED - Please fix before proceeding")
        return False
    elif failed_count > 0:
        print_warning(f"{failed_count} non-critical check(s) failed, but proceeding since they are not required.")
        return True
    else:
