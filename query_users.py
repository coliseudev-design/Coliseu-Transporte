...
res_users = run_query("SELECT id, email, nome, tenant_id::text, role, ativo FROM dash_usuarios")
print("=== DASHBOARD USERS ===")
if res_users:
    for row in res_users:
        print(f"ID: {row['id']} | Email: {row['email']} | Nome: {row['nome']} | Tenant: {row['tenant_id']} | Role: {row['role']} | Active: {row['ativo']}")
